...
# Data input user selalu ada di sini
...
if st.sidebar.button("Predict Profitability"):
    ...
    encoded_df['RestaurantID'] = encoded_df['RestaurantID'].map(mapping_restaurant)
    encoded_df['MenuCategory'] = encoded_df['MenuCategory'].map(mapping_category)
    encoded_df['MenuItem'] = encoded_df['MenuItem'].map(mapping_item)

    # Scaling harga (dummy scaling — ganti dengan nilai training asli)
    price_mean = 0.0   # isi sesuai training
    price_std = 1.0    # isi sesuai training
    encoded_df['Price'] = (encoded_df['Price'] - price_mean) / price_std
